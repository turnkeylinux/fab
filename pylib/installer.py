        ...
        for packages in (high, regular):
            if packages:
                try:
                    ...
                    output = file(join(self.chroot.path, "var/log/apt/term.log")).readlines()
                    output.reverse()
                    for line in output:
                        line = line.strip()
                        if line == "Errors were encountered while processing:":
                            for error in errors:
                                if error in ignore_errors:
                                    print "Warning: Ignoring errors generated by: %s" % error
                                    continue

                                raise executil.ExecError(e.command, e.exitcode, e.output)
